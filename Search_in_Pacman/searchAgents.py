...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize corner state as False for unvisited.
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        cornerState = state[1]
        # If there is no False in corner state, it means all the corner codes are visited.
        isGoal = not (False in cornerState)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                succCorner = list(cornerState)
                nextState = (nextx, nexty)
                cost = 1
                # If next state is one of corner, then update that corner state to True.
                if nextState in self.corners:
                    index = self.corners.index(nextState)
                    succCorner[index] = True
                successor = ((nextState, succCorner), action, cost)
...
def cornersHeuristic(state, problem):
    ...
    # For goal statre the heuristic value is 0.
    if problem.isGoalState(state):
        return 0

    unvisitedCorner = []
    for i in range(len(corners)):
        if visitedCorner[i] == False:
            unvisitedCorner.append(corners[i])

    # Choose the corner which has the maximum manhanttan distance to the
    # current position and return this distance as the heuristic value.
    closestCorner = []
    for corner in unvisitedCorner:
        distance = util.manhattanDistance(node, corner)
        closestCorner.append((distance, corner))
    closestCorner.sort(reverse=True)
    maxDistance, corner = closestCorner[0]
...
def foodHeuristic(state, problem):
    ...
    # Choose the corner which has the maximum maze distance to the
    # current position and return this distance as the heuristic value.
    remaining = []
    for node in foodNode:
        distance = mazeDistance(node, currentNode, gameState)
        remaining.append((distance, node))
    remaining.sort(reverse=True)
    maxDistance, closestNode = remaining[0]
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # When it comes to AnyFoodSearchProblem, the each food coordinate can be a small goal.
